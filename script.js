/**
 * AURA 2.5 Aurum Edition by Nygma - Ultimate Core + Auth
 */
...
// Пинг сервера (с передачей данных пользователя)
function pingServer() {
    const data = {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    btnDownload.addEventListener('click', async () => {
        ...
        try {
            ...
            if (data && data.medias && data.medias.length > 0) {
                ...
                // Записываем историю с выбранным качеством
                saveHistory(targetUrl, `MP4 - ${q.toUpperCase()}`);
    ...
    // --- СЕКРЕТНЫЙ ДОСТУП В АДМИНКУ (Резервный) ---
    let badgeClicks = 0;
        ...
        try {
            ...
            if (statsData.sessions.length === 0) {
                ...
                statsData.sessions.forEach(s => {
                    const userName = s.username ? `<b>${escapeHTML(s.username)}</b>` : '<span style="color:#666;">Без регистрации</span>';
                    tbody.innerHTML += `
                        <tr>
